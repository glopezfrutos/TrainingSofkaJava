...
import static javalab.JavaLabMenu.play;
...
public class GameLogic {
    ...
    public static void newPlayer() {
        ...
        System.out.println("\nPor favor, introducí tu nombre para comenzar a jugar");
    ...
    public static String compareTo(ArrayList<Player> arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            if (arrayList.get(i).getHistoricalScore() > mayor.getHistoricalScore()) {
    ...
    /**
     * @return Devuelve un número aleatorio entre 0 y 4 para seleccionar una pregunta.
     */
    ...
    /**
     * Define qué hacer a partir de la opción elejida como respuesta por el usuario
     *
     * @param option   opción elegida por el usuario
     * @param question pregunta activa
     * @param player   jugador activo
     * @return true para salir del submenu
     */
    public static boolean handleAnswer(int option, Question question, Player player) {
        if (option != question.getCorrectAnswer()) {
            System.out.println("Respuesta incorrecta..." +
                    "\nHas perdido el premio acumulado.");
            player.resetLevelAndScore();
            return true;
        }
        if (player.getLevel() < 4) {
            ...
            System.out.println("¡Respuesta correcta!" +
                    "\nAvanzaste al siguiente nivel y acumulaste u$s " + player.getActualScore() + ".");
            play();
            return true;
        }
        player.scoreUp();
        System.out.println("¡Respuesta correcta!" +
                "\nCompletaste todos los niveles y ganaste u$s " + player.getActualScore() + ".");
        player.setHistoricalScore();
        player.resetLevelAndScore();
        return true;
    }
}
